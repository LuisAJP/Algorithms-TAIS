...
vector<bool> objetosSeleccionados(vector<int> const& P, const Matriz<int> &mochila, int M){
	int n = P.size() - 1;
...
//Coste: O(nM) tanto en tiempo como en espacio adicional.
int maximoValor(vector<int> const& P, vector<int> const& V, int M, Matriz<int> &mochila){
	int n = P.size() - 1;
...
//Coste: O(nM) tanto en tiempo como en espacio adicional.
int maximoValor2(vector<int> const& P, vector<int> const& V, int M, vector<int> &mochila){
	int n = P.size() - 1;
	//Matriz<int> mochila(N + 1, M + 1, 0);
	// rellenar la matriz
	mochila[0] = 0;
	for (int i = 1; i <= n; ++i) {
		for (int j = P[i]; j <= M; ++j) {
			mochila[j] = max(mochila[j], mochila[j - P[i]] + 1);
		}
	}
	return mochila[M];
}
...
bool resuelveCaso() {
	...
	cin >> M;
	Matriz<int> mochila(N + 1, M + 1, 0);
	vector<int> mochila2(M+1,0);
	int max = maximoValor(P,V,M,mochila);
	int max2 = maximoValor2(P, V, M, mochila2);
	vector<bool> cuales = objetosSeleccionados(P, mochila, M);
	return true;
}
